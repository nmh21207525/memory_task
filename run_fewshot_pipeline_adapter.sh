...
usage() {
  cat <<EOF
Usage:
  bash run_fewshot_pipeline_adapter.sh --model_path /path/to/model --dataset bbh|arc|password|mmlu [options]

Required:
  --model_path PATH
  --dataset bbh|arc|password|mmlu

Common options:
  --gpu_ids 0,1,2,3                # auto split 的候选卡池
  --train_gpu_ids 1,2,3            # 显式训练卡（可选）
  --inference_gpu_id 0             # 显式推理卡（默认 0）
...
Auto split policy (when train/inference not fully provided):
  4+ cards: infer=0, train=others
  3 cards : infer=0, train=1,2
  2 cards : infer=0, train=1
  1 card  : infer=0, train=0 (serial)
EOF
}
...
echo "Run few-shot adapter pipeline"
