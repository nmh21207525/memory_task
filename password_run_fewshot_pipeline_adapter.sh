...
usage() {
  cat <<EOF
Usage:
  bash password_run_fewshot_pipeline_adapter.sh --model_path /path/to/model [options]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --dataset) echo "This script is fixed to dataset=password. Please remove --dataset."; exit 1 ;;
...
echo "Run adapter pipeline (Password)"
