...
usage() {
  cat <<EOF
Usage:
  bash arc_run_fewshot_pipeline_adapter.sh --model_path /path/to/model [options]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --dataset) echo "This script is fixed to dataset=arc. Please remove --dataset."; exit 1 ;;
